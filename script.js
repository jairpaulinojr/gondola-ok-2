    ...
    .then(dados => {
        ...
        try {
            localStorage.setItem(
                "gondola_base_global",
                JSON.stringify(baseGlobalProdutos)
            );
...
function carregarPlanilhaSetor(inputElement, nomeSetor) {
    ...
    leitor.onload = function (evento) {
        ...
        let json = XLSX.utils.sheet_to_json(
            arquivoExcel.Sheets[arquivoExcel.SheetNames[0]]
        );

        missoesPorSetor[nomeSetor] = json
            .map((item, idx) => ({
                id: Date.now() + idx,
                codigo: String(
                    item["Cód. de Barras"] ||
        ...
        localStorage.setItem(
            "gondola_missoes_setores",
            JSON.stringify(missoesPorSetor)
        );
...
function abrirPainelAdmin() {
    ...
    document.querySelector(".container").innerHTML = `
        ${vencendoLogo.length > 0
            ? `<div style="background:#dc3545;color:white;padding:15px;text-align:center;">🚨 ${vencendoLogo.length} itens vencendo!</div>`
            : ""}
...
function voltarMenuPrincipal() {
    document.querySelector(".container").innerHTML = `
        <div class="topo">
            <h1>🛒 GÔNDOLA OK</h1>
            <p>Operador: <strong>${usuarioAtual}</strong></p>
        </div>
...
function buscarProdutoValidade(codigoBarras) {
    ...
    let produtoEncontrado = baseMestre.find(
        p => p.codigo === codigoBarras.trim()
    );
    ...
    if (!produtoEncontrado) {
        display.innerHTML = "❌ Produto não localizado.";
        return;
    }
    ...
    display.innerHTML = `
        ...
        <input type="date" id="data-venc">
        <input type="number" id="qtd-venc" value="1">

        <button onclick="salvarDataValidade('${produtoEncontrado.codigo}','${produtoEncontrado.descricao.replace(/'/g, "\\'")}')">
            Salvar
        </button>`;
}
        ...
        baseGlobalProdutos = json
            .map(item => {
                ...
                return {
                    codigo: String(codigoEncontrado).trim(),
                    descricao: String(descricaoEncontrada).trim(),
                    setor: String(item.setor || "Geral").trim()
                };
            })
            .filter(p => p.codigo !== "" && p.descricao !== "");
...
function mostrarTelaLoginInicial() {
    ...
    container.innerHTML = `
        ...
        <div class="login">
            ...
            <button onclick="entrar()" style="background-color:#28a745;color:white;">
                Entrar no Sistema
            </button>
        </div>
    `;
...
function abrirPainelAdmin() {
    ...
    let setoresLista = [
        ...
        { nome: "Mercearia Doce", icone: "🍬" },
        { nome: "Mercearia Conservas", icone: "🥫" },
        { nome: "Mercearia Alto Giro", icone: "🔄" },
        { nome: "Mercearia Limpeza", icone: "🧴" },
        { nome: "Frios Iogurte", icone: "🥛" },
        { nome: "Frios Congelados", icone: "🧊" },
        { nome: "Açougue", icone: "🥩" },
        { nome: "Padaria", icone: "🍞" }
    ];
...
function voltarMenuPrincipal() {
    ...
    container.innerHTML = `
        ...
        <div class="login">
            ...
            <button onclick="abrirAbaEtiquetas()" style="background:#ffc107;color:black;">
                🏷️ Etiquetas Pendentes
            </button>

            <button onclick="location.reload()" style="background:#6c757d;color:white;margin-top:15px;">
                ⬅️ Sair do App
            </button>
        </div>
    `;
...
function mostrarProdutoAtual() {
    ...
    <div class="login">
        ...
        <button id="btn-abs-sim"
            onclick="respostaAbastecido(true)"
            style="background:#e0e0e0;color:black;margin-bottom:8px;">
            🟢 SIM
        </button>

        <button id="btn-abs-nao"
            onclick="respostaAbastecido(false)"
            style="background:#e0e0e0;color:black;margin-bottom:8px;">
            🔴 NÃO
        </button>
        ...
        <div id="bloco-preco" style="display:none;">
            ...
            <button id="btn-prc-sim"
                onclick="respostaPrecificado(true)"
                style="background:#e0e0e0;color:black;margin-bottom:8px;">
                🟢 SIM
            </button>

            <button id="btn-prc-nao"
                onclick="respostaPrecificado(false)"
                style="background:#e0e0e0;color:black;margin-bottom:8px;">
                🔴 NÃO
            </button>
        ...
        <button id="btn-proximo"
            ...
            style="display:none;background:#28a745;color:white;width:100%;padding:10px;">
            ➡️ Avançar
        </button>
    </div>
    `;
...
function finalizarMissao() {
    ...
    container.innerHTML = `
        ...
        <div class="login" style="text-align:center;">
            ...
            <button onclick="voltarMenuPrincipal()"
...
function abrirAbaValidade() {
    ...
    container.innerHTML = `
        ...
        <div class="login" style="max-width:100%;">
            ...
            <button
                onclick="buscarProdutoValidade(document.getElementById('input-manual-code').value)"
                style="padding:8px;margin-top:5px;background:#6c757d;color:white;width:100%;">
                Buscar Manual
            </button>

            <div id="resultado-validade"
                 style="margin-top:20px;display:none;text-align:left;background:#f9f9f9;padding:15px;border-radius:5px;">
            </div>

            <br>

            <button
                onclick="voltarMenuPrincipal()"
...
function buscarProdutoValidade(codigoBarras) {
    ...
    if (!produtoEncontrado) {
        display.innerHTML = `
            <p style="color:red;text-align:center;">
                ❌ Produto não localizado no Cadastro Mestre Global Mensal.
            </p>
        `;
        return;
    }
    ...
    display.innerHTML = `
        ...
        <button
            onclick="salvarDataValidade('${produtoEncontrado.codigo}','${produtoEncontrado.descricao.replace(/'/g, "\\'")}')"
        ...
        linhas = `
            <tr>
                <td colspan="2" style="text-align:center;padding:20px;color:green;font-weight:bold;">
                    🎉 Nenhum produto sem preço pendente!
                </td>
            </tr>
        `;
    } else {
            ...
            linhas += `
                <tr style="border-bottom:1px solid #ddd;">
                    ...
                    <td style="padding:12px;text-align:center;width:90px;">
                        <button
                            onclick="darBaixaEtiquetaManual(${idx})"
                            style="background:#28a745;color:white;padding:8px 10px;font-size:12px;border:none;border-radius:4px;font-weight:bold;cursor:pointer;">
                            ✓ Feito
                        </button>
                    </td>
                </tr>
...
function abrirRelatorioValidadesGerente() {
    ...
    if (registros.length === 0) {
        linhas = `
            <tr>
                <td colspan="4" style="padding:20px;text-align:center;color:green;font-weight:bold;">
        ...
        registros.forEach(r => {
            ...
            linhas += `
                <tr style="${estiloTratado}border-bottom:1px solid #ddd;font-size:13px;">
                    ...
                    <td style="padding:10px;text-align:center;">
                        <button
                            onclick="executarAcao('${r.codigo}','${r.dataValidade}','Tratado')"
                            style="background:#007bff;color:white;border:none;padding:5px;margin:2px;cursor:pointer;">
                            Tratar
                        </button>

                        <button
                            onclick="executarAcao('${r.codigo}','${r.dataValidade}','Resolvido')"
                            style="background:#28a745;color:white;border:none;padding:5px;margin:2px;cursor:pointer;">
                            Resolver
                        </button>
                    </td>
                </tr>
    ...
    container.innerHTML = `
        ...
        <div class="login" style="max-width:100%;padding:10px;">
            ...
            <button
                onclick="exportarParaExcel('Relatorio_Validades','registro_validades')"
                style="width:100%;background:#6c757d;color:white;padding:10px;margin-bottom:10px;">
                📥 Exportar para Excel
            </button>
...
window.atualizarStatus = function(codigo, dataValidade, novoStatus) {
    ...
    let item = registros.find(
        r => r.codigo === codigo && r.dataValidade === dataValidade
    );
    ...
    if (item) {
        ...
        alert("Erro ao localizar o item.");
    }
};
// ==========================================
...
function abrirAbaGerente(setorFiltro) {
        ...
        linesTabela = `
            <tr>
                <td colspan="3" style="padding:15px;text-align:center;color:#666;">
                    Nenhum item vistoriado hoje neste setor.
                </td>
            </tr>
        `;
    } else {
        itensVistoriadosDoSetor.forEach(p => {
            let statusAbs = p.abastecido
                ? "<span style='color:green;font-weight:bold;'>🟢 OK</span>"
                : "<span style='color:red;font-weight:bold;'>🔴 RUPTURA</span>";

            let statusPrc = p.precificado === true
                ? "🟢 Sim"
                : (p.precificado === false ? "🔴 Não" : "⚠️ N/A");

            linesTabela += `
                <tr style="border-bottom:1px solid #ddd;font-size:12px;">
                    <td style="padding:10px;">
                        ${p.descricao}<br>
                        <small style="color:#777;">Cód: ${p.codigo}</small>
                    </td>
                    ...
                    <td style="padding:10px;">
                        ${p.statusRuptura ? `
                            <select onchange="salvarDecisaoGerente('${setorFiltro}', ${p.id}, this.value)"
                                style="padding:4px;font-size:11px;width:100%;">
    ...
    container.innerHTML = `
        ...
        <div class="login" style="max-width:100%;padding:10px;">
            ...
            <button
                onclick="abrirMenuSetoresGerente()"
                style="background:#6c757d;color:white;width:100%;">
                Voltar aos Setores
            </button>
        </div>
...
function abrirGestaoEtiquetas() {
        ...
        etiquetas.forEach((e, index) => {
            linhas += `
                <tr style="border-bottom:1px solid #ddd;font-size:13px;">
                    ...
                    <td style="padding:10px;text-align:center;">
                        <button
                            onclick="removerEtiqueta(${index})"
                            style="background:#28a745;color:white;border:none;padding:6px;border-radius:3px;cursor:pointer;">
                            Resolvido
                        </button>
                    </td>
                </tr>
    ...
    container.innerHTML = `
        ...
        <div class="login" style="max-width:100%;padding:10px;">
            ...
            <button
                onclick="gerarEtiquetasCodigoBarras(JSON.parse(localStorage.getItem('etiquetas_pendentes') || '[]'))"
            ...
            <button
                onclick="abrirMenuSetoresGerente()"
                style="background:#6c757d;color:white;width:100%;padding:10px;">
                ⬅️ Voltar ao Menu
            </button>
        </div>
    `;
...
window.gerarEtiquetasCodigoBarras = function(lista) {
    ...
    lista.forEach(p => {
        html += `
            <div class="etiqueta">
                <strong>${String(p.descricao).replace(/</g, "&lt;")}</strong><br>
                <svg id="b${p.codigo}"></svg>
            </div>
        `;
    });
    ...
    html += `
        ...
        <script>
            window.onload = function () {
                ${lista.map(p =>
                    `JsBarcode("#b${p.codigo}", "${p.codigo}", { width:2, height:40, fontSize:12 });`
                ).join("\n")}
...
// ==========================================
// CORREÇÃO: Função buscarProdutoValidade
// ==========================================
function buscarProdutoValidade(codigoBarras) {
    // Tenta pegar do localStorage; se estiver vazio, usa a base global carregada
    let baseMestre = JSON.parse(localStorage.getItem("gondola_base_global") || "[]");

    if (baseMestre.length === 0) {
        baseMestre = baseGlobalProdutos;
    }
